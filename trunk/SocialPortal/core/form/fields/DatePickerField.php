...
class DatePickerField extends Field {
	...
	public function insertLabel(){
		?><label for="<?php echo $this->identifier; ?>_day"><?php echo $this->description ?></label><?php
	}
...
//		$yearId = $this->identifier . '_year';
		...
		?>
		<select id="<?php echo $dayId ?>" name="<?php echo $dayName ?>" class="<?php echo $this->getConstraintsAsString();?>">
			<option <?php if(0==$selected[0]) echo 'selected ' ?>value="">--</option>
			<?php for($i = 1; $i <= 31 ; $i++): ?>
				<option <?php if($i==$selected[0]) echo 'selected ' ?>value="<?php echo $i; ?>"><?php echo $i; ?></option>
			<?php endfor; ?></select>
		<select id="<?php echo $monthId ?>" name="<?php echo $monthName ?>" class="<?php echo $this->getConstraintsAsString();?>">
			<option <?php if(0==$selected[1]) echo 'selected ' ?>value="">------</option>
			<?php for($i = 1; $i <= 12 ; $i++): ?>
				<option <?php if($i==$selected[1]) echo 'selected ' ?>value="<?php echo $i; ?>"><?php echo $months[$i-1]; ?></option>
			<?php endfor; ?></select>
		<select id="<?php echo $yearId ?>" name="<?php echo $yearName ?>" class="<?php echo $this->getConstraintsAsString();?>">
			<option <?php if(0==$selected[2]) echo 'selected ' ?>value="">----</option>
			<?php for($i = 112; $i >= 1 ; $i--): ?>
				<option <?php if($i==$selected[2]) echo 'selected ' ?>value="<?php echo ($startYear+$i); ?>"><?php echo ($startYear+$i); ?></option>
			<?php endfor; ?></select>
	<?php
	}
